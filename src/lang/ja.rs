...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("install_tip", "Heko RemotesupportがUACの影響によりリモート側で正常に動作しない場合があります。UACを回避するには、下のボタンをクリックしてシステムにHeko Remotesupportをインストールしてください。"),
        ("Click to upgrade", "アップグレード"),
        ("Click to download", "ダウンロード"),
        ("Click to update", "アップデート"),
        ("Configure", "設定"),
        ("config_acc", "リモートからあなたのデスクトップを操作するには、Heko Remotesupportに「アクセシビリティ」権限を与える必要があります。"),
        ("config_screen", "リモートからあなたのデスクトップにアクセスするには、Heko Remotesupportに「画面収録」権限を与える必要があります。"),
        ("Installing ...", "インストール中..."),
        ("Install", "インストール"),
        ("Installation", "インストール"),
        ...
        ("android_input_permission_tip1", "このAndroid端末をリモートの端末からマウスやタッチで操作するには、Heko Remotesupportに「アクセシビリティ」サービスの使用を許可する必要があります。"),
        ...
        ("Language", "言語"),
        ("Keep Heko Remotesupport background service", "Heko Remotesupport バックグラウンドサービスを維持"),
